...
export function BulkTranslationManager({ className }: BulkTranslationManagerProps) {
    ...
    contents.forEach((content) => {
      ...
      ALL_LANGUAGES.forEach((lang) => {
        const translation = translations.find(
          (t) => t.contentId === content.id && t.locale === lang.code
        );
        matrix[content.id][lang.code] = translation?.status || "missing";
      });
    });
  ...
  const stats = useMemo(() => {
    ...
    filteredContents.forEach((content) => {
      ALL_LANGUAGES.forEach((lang) => {
        total++;
        const status = translationMatrix[content.id]?.[lang.code];
        if (status === "completed") {
          translated++;
        } else {
          missing++;
  ...
  const selectMissingOnly = () => {
    ...
    filteredContents.forEach((content) => {
      ALL_LANGUAGES.forEach((lang) => {
        if (translationMatrix[content.id]?.[lang.code] !== "completed") {
          newContents.add(content.id);
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Select content and languages to translate in bulk
            </CardDescription>
          </div>
            ...
            {filteredContents.map((content) => (
              <div
                ...
                className={`flex items-center gap-2 p-2 rounded-lg transition-colors ${
                  selectedContents.has(content.id) ? "bg-primary/10" : "hover:bg-muted/50"
                }`}
              >
                <Checkbox
                ...
                <div className="flex items-center gap-2">
                  {ALL_LANGUAGES.map((lang) => {
                    const status = translationMatrix[content.id]?.[lang.code] || "missing";
                    const isSelected =
                      selectedContents.has(content.id) && selectedLocales.has(lang.code);
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to translate {translationsToCreate} content items. This action will use
