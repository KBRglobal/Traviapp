...
interface ContentCalendarProps {
  onSelectContent?: (content: ContentWithRelations) => void;
}
...
export function ContentCalendar({ onSelectContent }: ContentCalendarProps) {
  ...
  const contentByDate = useMemo(() => {
    ...
    contents.forEach((content) => {
      // Filter by type
      if (filterType !== "all" && content.type !== filterType) return;
      // Filter by status
      if (filterStatus !== "all" && content.status !== filterStatus) return;

      // Use scheduledAt for scheduled content, or updatedAt for others
      const dateField = content.status === "scheduled" && content.scheduledAt
  ...
  const handleContentClick = (content: ContentWithRelations) => {
    if (onSelectContent) {
                                ...
                                <TooltipContent side="right" className="max-w-[250px]">
                                  <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-2 text-xs">
                                      <Badge className={statusColors[content.status || "draft"]}>
                                        {content.status}
                                      </Badge>
                                    ...
                                    {content.scheduledAt && (
                                      <p className="text-xs text-muted-foreground">
                                        Scheduled: {new Date(content.scheduledAt).toLocaleString()}
                                      </p>
                                    )}
                                  </div>
                                </TooltipContent>
