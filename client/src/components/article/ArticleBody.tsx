  ...
  return (
    ...
    >
      <p className="text-lg italic text-muted-foreground leading-relaxed">
        {data.quote || data.content || data.text}
      </p>
      {data.author && (
...
function renderTips(data: Record<string, any>, index: number) {
  // Handle both array format (legacy) and content string format (new)
  let tips: string[] = [];
  if (typeof data.content === "string" && data.content.trim()) {
    tips = data.content.split("\n").filter((line: string) => line.trim());
  } else if (Array.isArray(data.tips)) {
    tips = data.tips.map((item: any) => typeof item === "string" ? item : item.text || item.content || "");
  } else if (Array.isArray(data.items)) {
    tips = data.items.map((item: any) => typeof item === "string" ? item : item.text || item.content || "");
  }
  ...
  return (
    <div
      key={index}
      className="my-8 p-6 rounded-lg bg-gradient-to-br from-purple-50 to-pink-50/50 dark:from-purple-950/30 dark:to-pink-950/20 border border-purple-100/50 dark:border-purple-800/30"
