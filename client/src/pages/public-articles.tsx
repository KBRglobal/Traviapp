...
function ArticleCard({ content, index, t, localePath }: { content: ContentWithRelations; index: number; t: (key: string) => string; localePath: (path: string) => string }) {
  ...
  return (
    <Link href={localePath(`/articles/${content.slug}`)}>
      <Card
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredArticles.map((article, index) => (
                <ArticleCard key={article.id} content={article} index={index} t={t} localePath={localePath} />
              ))}
            </div>
          ) : (
            <div className="text-center py-16">
              ...
              <h3 className="text-xl font-semibold mb-2">{t("articlesPage.noArticlesFound")}</h3>
