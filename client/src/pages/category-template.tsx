...
function ContentCard({ content, localePath, isRTL }: { content: Content; localePath: (path: string) => string; isRTL: boolean }) {
  const config = categoryConfig[content.type as CategoryType] || categoryConfig.hotels;
  const image = content.featuredImage || content.heroImage || config.defaultImage;
  const href = localePath(`/${content.type}s/${content.slug}`);
      ...
      >
        ...
        >
          <div className="aspect-[4/3] overflow-hidden relative">
            ...
            {content.hotel?.starRating && (
              <div className={cn("absolute top-3 flex items-center gap-1", isRTL ? "right-3" : "left-3")}>
                <Badge className="bg-travi-orange/90 text-white border-0">
          ...
          <CardContent className={cn("p-4 space-y-2", isRTL && "text-right")}>
            ...
            {(content.hotel?.location || content.dining?.location) && (
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <MapPin className="w-3 h-3" />
                <span>{content.hotel?.location || content.dining?.location}</span>
              </div>
            )}
...
function ContentGrid({
  ...
  if (isLoading) {
    return (
      <section className="py-16" data-testid="content-grid-loading">
        <div className="container mx-auto px-4">
  ...
  if (contents.length === 0) {
    return (
      <section className="py-16" data-testid="content-grid-empty">
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {contents.map((content) => (
            <ContentCard key={content.id} content={content} localePath={localePath} isRTL={isRTL} />
          ))}
        </div>
      </div>
    </motion.section>
