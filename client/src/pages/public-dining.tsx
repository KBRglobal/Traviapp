...
import { Utensils, Star, MapPin, ArrowLeft, Search } from "lucide-react";
...
function DiningCard({ content, index }: { content: Content; index: number }) {
      ...
      <Card className="group overflow-hidden border-0 shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer">
        ...
        <div className="p-5">
          ...
          <p className="text-sm text-muted-foreground line-clamp-2 mb-4">
            {content.metaDescription || "Experience world-class cuisine in the heart of Dubai."}
          </p>
...
function PlaceholderDiningCard({ index }: { index: number }) {
  const placeholderData = [
    { title: "At.mosphere", desc: "World's highest restaurant in the Burj Khalifa with stunning views" },
    { title: "Pierchic", desc: "Overwater seafood restaurant on a pier at Al Qasr hotel" },
    { title: "Zuma Dubai", desc: "Contemporary Japanese cuisine in DIFC" },
    { title: "La Petite Maison", desc: "Refined French-Mediterranean dining experience" },
  ];
...
export default function PublicDining() {
  ...
  const diningItems = allContent?.filter(c => c.type === "dining") || [];
  const filteredDining = searchQuery 
    ? diningItems.filter(d => d.title.toLowerCase().includes(searchQuery.toLowerCase()))
    : diningItems;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              <Link href="/dining" className="text-[#6443f4] font-medium">Dining</Link>
      ...
      <section className="bg-gradient-to-br from-[#6443f4] via-[#7c5cf7] to-[#9b7bfa] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold text-white">Dining in Dubai</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-6">
            <p className="text-muted-foreground">
              {isLoading ? "Loading..." : `${filteredDining.length} restaurants found`}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              ...
              filteredDining.map((item, index) => (
                <DiningCard key={item.id} content={item} index={index} />
              ))
            ) : (
              [0, 1, 2, 3].map((index) => (
                <PlaceholderDiningCard key={index} index={index} />
              ))
            )}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 text-muted-foreground text-sm">
              ...
              <Link href="/dining" className="hover:text-foreground transition-colors">Dining</Link>
